...
(function () {
  ...
  window.auditSlides = function auditSlides(tolerancePx) {
    ...
    // headless 取用：把結果塞進 title（只在 headless／?audit 時，免得一般瀏覽器分頁標題與存檔的 title 被 JSON 蓋掉）
    try { if (navigator.webdriver || /[?&]audit/.test(location.search)) document.title = JSON.stringify(results); } catch (e) {}
